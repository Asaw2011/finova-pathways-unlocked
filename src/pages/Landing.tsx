    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center"><img src="/finova-logo-horizontal.png" alt="FinOva" className="h-12 sm:h-14" /></Link>
      ...
      {/* FEATURES */}
      <div id="features" className="py-16 sm:py-24 space-y-20 sm:space-y-32 px-4 sm:px-6">
        {features.map(({ headline, body, bullets, image, alt }, i) => {
          ...
          return (
            <S key={i} className="max-w-6xl mx-auto">
              <div className={`grid md:grid-cols-2 gap-8 sm:gap-12 items-center ${i % 2 === 1 ? "md:[direction:rtl] md:*:[direction:ltr]" : ""}`}>
                <div className={textAnim}>
                  <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-2 sm:mb-3">FEATURE {i + 1} OF {features.length}</p>
                  <h3 className="text-xl sm:text-2xl md:text-4xl font-black mb-3 sm:mb-4" style={{ fontFamily: "var(--font-display)" }}>{headline}</h3>
                  <p className="text-sm sm:text-base text-muted-foreground mb-4 sm:mb-6 leading-relaxed">{body}</p>
                  <ul className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">{bullets.map((b) => (<li key={b} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm"><div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-primary/10 flex items-center justify-center mt-0.5 shrink-0"><CheckCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-primary" /></div>{b}</li>))}</ul>
                  <Link to="/auth"><Button className="btn-cinema bg-primary text-primary-foreground rounded-xl font-bold text-sm">Try it free <ArrowRight className="ml-2 w-4 h-4" /></Button></Link>
                </div>
      ...
      {/* HOW IT WORKS */}
      <S className="py-16 sm:py-24 px-4 sm:px-6 bg-card border-y border-border texture-dots" id="how-it-works">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10 sm:mb-16 reveal">
            <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-3">GET STARTED</p>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black mb-3 sm:mb-4" style={{ fontFamily: "var(--font-display)" }}>Three steps. Sixty seconds.</h2>
            <p className="text-sm sm:text-base text-muted-foreground">No downloads. No credit card. No catch.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 stagger">
            {[{ step: "01", icon: Users, title: "Create your account", desc: "Sign up with email or Google. It takes about 10 seconds and you're in." },{ step: "02", icon: Flame, title: "Take your first lesson", desc: "Jump into Module 1 or let our quick assessment find your starting point." },{ step: "03", icon: Trophy, title: "Earn as you learn", desc: "Collect gems, badges, certificates, and climb the leaderboard as you level up." }].map(({ step, icon: Icon, title, desc }) => (
              <div key={step} className="reveal text-center">
                <p className="text-4xl sm:text-5xl font-black text-primary/20 mb-2" style={{ fontFamily: "var(--font-display)" }}>{step}</p>
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-3 sm:mb-4"><Icon className="w-6 h-6 sm:w-7 sm:h-7 text-primary" /></div>
          ...
          <div className="text-center mt-10 sm:mt-12 reveal">
            <Link to="/auth"><Button size="lg" className="w-full sm:w-auto btn-cinema bg-primary text-primary-foreground rounded-2xl font-bold px-8 py-5 sm:py-6 text-sm sm:text-base">Start Learning Now <ArrowRight className="ml-2 w-5 h-5" /></Button></Link>
          </div>
        </div>
      </S>

      {/* REVIEWS */}
      <S className="py-16 sm:py-24 px-4 sm:px-6" id="reviews">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16 reveal">
            <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-3">REVIEWS</p>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black mb-4" style={{ fontFamily: "var(--font-display)" }}>Loved by students, parents,{" "}<span className="gradient-text">and educators.</span></h2>
            <div className="flex items-center justify-center gap-2 mt-4">
              <div className="flex">{[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 sm:w-5 sm:h-5 fill-[hsl(var(--duo-gold))] text-[hsl(var(--duo-gold))]" />)}</div>
              <span className="font-bold text-base sm:text-lg">5.0</span>
              <span className="text-xs sm:text-sm text-muted-foreground">from {reviews.length} reviews</span>
            </div>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 stagger">
            {reviews.map(({ q, n, r, a }) => (
              <div key={a} className="reveal card-3d">
                <div className="card-3d-inner p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-card border border-border h-full flex flex-col">
                  <div className="flex gap-0.5 mb-2 sm:mb-3">{[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-[hsl(var(--duo-gold))] text-[hsl(var(--duo-gold))]" />)}</div>
                  <p className="text-xs sm:text-sm leading-relaxed flex-1 mb-3 sm:mb-4">&ldquo;{q}&rdquo;</p>
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-primary/10 flex items-center justify-center text-[10px] sm:text-xs font-bold text-primary">{a.slice(0, 2)}</div>
                    <div>
                      <p className="text-xs sm:text-sm font-bold">{n}</p>
                      <p className="text-[10px] sm:text-xs text-muted-foreground">{r}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* PRICING */}
      <S className="py-16 sm:py-24 px-4 sm:px-6 bg-card border-y border-border texture-grid" id="pricing">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 sm:mb-16 reveal">
            <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-3">PRICING</p>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black mb-4" style={{ fontFamily: "var(--font-display)" }}>Free forever. Plus if you want more.</h2>
            <p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto">Every core feature is free. FinOva Plus unlocks premium perks for power learners.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
            <div className="reveal p-6 sm:p-8 rounded-2xl sm:rounded-3xl bg-background border border-border">
              <h3 className="text-xl sm:text-2xl font-black mb-1" style={{ fontFamily: "var(--font-display)" }}>Free</h3>
              <p className="text-muted-foreground text-xs sm:text-sm mb-3 sm:mb-4">Everything you need to get started</p>
              <p className="text-3xl sm:text-4xl font-black mb-4 sm:mb-6" style={{ fontFamily: "var(--font-display)" }}>$0 <span className="text-sm sm:text-base font-normal text-muted-foreground">forever</span></p>
              <Link to="/auth"><Button className="w-full rounded-xl font-bold mb-4 sm:mb-6" variant="outline">Get Started Free</Button></Link>
              <ul className="space-y-2.5 sm:space-y-3">{["All 10 learning modules", "80+ interactive lessons", "8 financial games", "Paper trading simulator", "AI money coach (limited)", "Earn gems & badges", "5 hearts per day", "Basic certificates"].map((t) => (<li key={t} className="flex items-center gap-2 text-xs sm:text-sm"><CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary shrink-0" />{t}</li>))}</ul>
            </div>
            <div className="reveal gradient-border p-6 sm:p-8 rounded-2xl sm:rounded-3xl bg-background">
              <div className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-[10px] sm:text-xs font-bold mb-3 sm:mb-4">MOST POPULAR</div>
              <h3 className="text-xl sm:text-2xl font-black mb-1" style={{ fontFamily: "var(--font-display)" }}>FinOva <span className="gradient-text">Plus</span></h3>
              <p className="text-muted-foreground text-xs sm:text-sm mb-3 sm:mb-4">For serious learners who want it all</p>
              <p className="text-3xl sm:text-4xl font-black mb-1" style={{ fontFamily: "var(--font-display)" }}>$9.99 <span className="text-sm sm:text-base font-normal text-muted-foreground">/month</span></p>
              <p className="text-[10px] sm:text-xs text-muted-foreground mb-4 sm:mb-6">Cancel anytime &middot; No commitment</p>
              <Link to="/plus"><Button className="w-full btn-cinema bg-primary text-primary-foreground rounded-xl font-bold mb-4 sm:mb-6">Upgrade to Plus <ArrowRight className="ml-2 w-4 h-4" /></Button></Link>
              <p className="text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 sm:mb-3">Everything in Free, plus:</p>
              <ul className="space-y-2.5 sm:space-y-3">{[{ i: Heart, t: "Unlimited hearts" },{ i: BookOpen, t: "All premium courses" },{ i: Zap, t: "1,000 bonus gems/month" },{ i: Shield, t: "3 streak freezes/month" },{ i: Star, t: "Exclusive badges & certs" },{ i: Crown, t: "Mistakes review mode" },{ i: Lock, t: "Unlimited AI coach" },{ i: Award, t: "Priority support" }].map(({ i: Icon, t }) => (<li key={t} className="flex items-center gap-2 text-xs sm:text-sm"><Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary shrink-0" />{t}</li>))}</ul>
            </div>
          </div>
        </div>
      </S>

      {/* FFI + Investing Timeline */}
      <S className="py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <div className="mb-12 sm:mb-16 max-w-md sm:max-w-lg mx-auto">
            <InvestingTimeline />
          </div>
          <div className="text-center reveal">
            <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-3">OUR PARTNER</p>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black mb-4 sm:mb-6" style={{ fontFamily: "var(--font-display)" }}>Financial Freedom Initiative</h2>
            <p className="text-sm sm:text-base text-muted-foreground leading-relaxed mb-6 sm:mb-8 max-w-2xl mx-auto">FFI is a student-led nonprofit bringing financial literacy to high schoolers nationwide through workshops, mentorship, and real-world education programs. FinOva is the digital extension of their mission.</p>
            <a href="https://www.instagram.com/financial.freedom.initiative?igsh=MW5jNGVkOWppZW1yaQ%3D%3D&utm_source=qr" target="_blank" rel="noopener noreferrer">
              <Button variant="outline" className="rounded-xl font-bold text-sm">Follow FFI on Instagram <ExternalLink className="ml-2 w-4 h-4" /></Button>
            </a>
          </div>
        </div>
      </S>
      ...
      {/* FAQ */}
      <S className="py-16 sm:py-24 px-4 sm:px-6 bg-card border-y border-border" id="faq">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-8 sm:mb-12 reveal">
            <p className="text-xs sm:text-sm font-bold text-primary tracking-widest uppercase mb-3">FAQ</p>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black" style={{ fontFamily: "var(--font-display)" }}>Questions? Answers.</h2>
          </div>
      ...
      {/* FINAL CTA */}
      <S className="relative py-20 sm:py-32 px-4 sm:px-6 overflow-hidden">
        <div className="absolute inset-0 z-0">
      ...
      {/* FOOTER */}
      <footer className="py-10 sm:py-16 px-4 sm:px-6 border-t border-border bg-card">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 mb-8 sm:mb-12">
            ...
            <div>
              <h5 className="font-bold text-xs sm:text-sm mb-2 sm:mb-3">Platform</h5>
            ...
            <div>
              <h5 className="font-bold text-xs sm:text-sm mb-2 sm:mb-3">Company</h5>
              <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-muted-foreground">
                <Link to="/about" className="block hover:text-foreground transition-colors">About Us</Link>
                <a href="https://www.instagram.com/financial.freedom.initiative?igsh=MW5jNGVkOWppZW1yaQ%3D%3D&utm_source=qr" target="_blank" rel="noopener noreferrer" className="block hover:text-foreground transition-colors">FFI Partnership</a>
            ...
            <div>
              <h5 className="font-bold text-xs sm:text-sm mb-2 sm:mb-3">For Students</h5>
              <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-muted-foreground">
          ...
          <div className="border-t border-border pt-6 sm:pt-8 text-center">
            <p className="text-[10px] sm:text-xs text-muted-foreground">&copy; {new Date().getFullYear()} FinOva. All rights reserved.</p>
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Built in partnership with the Financial Freedom Initiative.</p>
          </div>
        </div>
      </footer>
    </div>
